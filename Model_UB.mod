...
subject to Norme {i in 1..M, t in 1..T}:
	...
	(1/(2*NM[i]))*(sum{k in 1..floor(NM[i]/2)} (beta0[i,k])^2 + sum{k in a0..floor(NM[i]/2)} 2*(beta1[i,k]/2 - beta2[i])^2)
	) - ell[t]);

subject to Norme_MinMax {i in 1..M}:
	Phi >= (1/(2*NM[i]))*(sum{k in 1..floor(NM[i]/2)} (beta0[i,k])^2 + sum{k in a0..floor(NM[i]/2)} 2*(beta1[i,k]/2 - beta2[i])^2);
...
subject to Beta1_Even {i in 1..M, k in 1..floor(NM[i]/2)}:
	beta1[i,k] =  sum{j in 0..(floor(NM[i]/2)-1)} x[i,NM[i] - 2*j] 
				+ sum{j in 1..k} x[i,2*j-1]
				+ sum{j in 0..(floor(NM[i]/2) - k)} x[i,NM[i] - 2*j];

subject to Beta1_Odd {i in 1..M, k in 0..floor(NM[i]/2)}:
	beta1[i,k] =  sum{j in 0..floor(NM[i]/2)} x[i,NM[i] - 2*j] 
				+ sum{j in 1..k} x[i,2*j]
				+ sum{j in 0..(floor(NM[i]/2) - k)} x[i,NM[i] - 2*j];

subject to Beta2_Odd {i in 1..M}:
	beta2[i] = (1/NM[i])*sum{k in 1..floor(NM[i]/2), j in 0..(floor(NM[i]/2) - k)} x[i,NM[i] - 2*j] +
				(1/NM[i])*sum{k in 1..floor(NM[i]/2), j in 1..k} x[i,2*j] +
				((floor(NM[i]/2)+1)/NM[i])*sum{j in 0..floor(NM[i]/2)} x[i,2*j+1] ;

subject to Beta2_Even {i in 1..M}:
	beta2[i] = (1/NM[i])*sum{k in 1..floor(NM[i]/2), j in 0..(k-1)} x[i,NM[i] - 2*j] +
				(1/NM[i])*sum{k in 1..floor(NM[i]/2), j in 1..k} x[i,2*j-1] +
				(1/2)*sum{j in 0..(floor(NM[i]/2)-1)} x[i,NM[i] - 2*j] ;
